/**
 * Seed the 4 flagship 1-year premium programs and their module breakdown from
 * the FSA curriculum document.
 *
 * Source: src/scripts/data/oneYearModules.txt (text extracted from
 *         "1_Year_Program_Detailed_Modules.docx").
 *
 * - Each program → one Course document + twelve Module documents.
 ...
 *   "1 Year". The original sequence is preserved via each module's `order`.
 ...
 *   SEED_PUBLISH=true.
 *
 * Run from the backend folder:  node src/scripts/seedOneYearCourses.js
 */
...
const DATA_FILE = path.join(__dirname, "data", "oneYearModules.txt");
...
function parseCourses(text) {
  ...
  // A program header looks like "#1 — AI Software Engineer Pro".
  const isHeader = (l) => /^#\d+\s*[—–-]\s*\S/.test(l.trim());
  const labelValue = (block, label) => {
    for (const l of block) {
      const t = l.trim();
      if (t.toLowerCase().startsWith(label.toLowerCase())) {
        return t.slice(label.length).replace(/^[:\s]+/, "").trim();
      }
    }
    return "";
  };
  ...
  for (let h = 0; h < headerIdx.length; h++) {
    ...
    const name = block[0].trim().replace(/^#\d+\s*[—–-]\s*/, "").trim();
...
async function run() {
  ...
  for (const c of courses) {
    ...
    const description =
      `${c.name} is an intensive 1-year premium program in ${c.category}. ` +
    ...
    // Guard against clobbering a shorter-tier course that happens to share a slug.
